...
export function Relatorios({ t }: { t: Locale }) {
  const r = t.relatorios;
  return (
    <Section label={t.sectionLabels.reports} heading={r.heading}>
      <DeviceRow>
        <IOSDevice>
          <div style={sx(PHONE_BODY)}>
            <div style={sx(PHONE_HEADER)}>
              <div style={sx("font-family:'Cormorant Garamond',serif; font-size:26px; font-weight:600;")}>{r.title}</div>
            </div>
            <div style={sx('flex:1; overflow:auto; padding:0 24px 24px; box-sizing:border-box; display:flex; flex-direction:column; gap:16px;')}>
              ...
              <div style={sx('background:#FFFFFF; border:1px solid #EBE2CF; border-radius:20px; padding:20px;')}>
                <div style={sx(LABEL_KICKER)}>{r.topServicesLabel}</div>
                {r.topServices.map((s) => (
