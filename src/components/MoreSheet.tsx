  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(27,23,18,0.35)', zIndex: 30, display: 'flex', alignItems: 'flex-end' }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', background: T.bg, borderRadius: '22px 22px 0 0', padding: '18px 20px 28px' }}>
        ...
        <div style={{ fontFamily: 'Playfair Display', fontSize: 17, color: T.ink, marginBottom: 14 }}>{t.nav.moreTitle}</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {items.map((m) => (
            <div
              ...
              style={{ border: `1px solid ${T.line}`, borderRadius: RADIUS.card, padding: 14, cursor: 'pointer', background: T.surface, boxShadow: SHADOW.card }}
            >
              ...
              <div style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 13, color: T.ink }}>{m.label}</div>
              <div style={{ fontFamily: 'Inter', fontSize: 11, color: T.muted }}>{m.sub}</div>
            </div>
          ))}
        </div>
      </div>
