...
export function InventoryScreen({
  ...
  return (
    <div style={sx(`${SCREEN_SCROLL}`)}>
      <ScreenHeader title={inv.title} backLabel={t.tabbar.mais} onBack={onBack} />
      ...
      <div style={sx(`${CONTENT_PAD_X} ${SCREEN_PAD_BOTTOM} box-sizing:border-box; display:flex; flex-direction:column; gap:14px;`)}>
        ...
        {inventoryHasLow && (
          <div style={sx('background:#201C17; border-radius:20px; padding:20px; color:#F4E9D2;')}>
            <div style={sx('font-size:12px; letter-spacing:1px; text-transform:uppercase; color:#C9A24B; font-weight:700; margin-bottom:12px;')}>{inventoryLowHeadline}</div>
            {inventoryLowProducts.map((p) => (
              ...
              >
                ...
                <div style={sx('cursor:pointer; flex-shrink:0; padding:9px 16px; border-radius:999px; background:#F4E9D2; color:#201C17; font-size:12px; font-weight:700;')}>
                  {inv.restockCta}
                </div>
              </div>
            ))}
        ...
        <div style={sx('background:#FFFFFF; border:1px solid #EBE2CF; border-radius:16px; padding:18px;')}>
          <div style={sx('font-size:11px; letter-spacing:1px; text-transform:uppercase; font-weight:700; color:#8A8074; margin-bottom:10px;')}>{inv.movementsLabel}</div>
          {inv.movements.map((m) => (
        ...
        <div style={sx('background:#FFFFFF; border:1px solid #EBE2CF; border-radius:16px; padding:18px;')}>
          <div style={sx('font-size:11px; letter-spacing:1px; text-transform:uppercase; font-weight:700; color:#8A8074; margin-bottom:10px;')}>{inv.consumptionLabel}</div>
        ...
        <div>
          <div style={sx('font-size:11px; letter-spacing:1px; text-transform:uppercase; font-weight:700; color:#8A8074; margin-bottom:10px;')}>{inv.allProductsLabel}</div>
